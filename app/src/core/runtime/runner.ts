...
function evaluateWhen(edge: ExecEdge, state: StateStore): boolean {
  ...
  const left = String(isStateRef(when.left) ? (state.get(when.left) ?? '') : when.left);
  const right = String(isStateRef(when.right) ? (state.get(when.right) ?? '') : when.right);
  switch (when.op) {
    case 'eq':
      return left === right;
    case 'neq':
...
export class Runner {
  ...
  async execute(
    ...
    while (queue.length > 0) {
      ...
      try {
        ...
        if (node.op === 'SWITCH') {
          selectedEdges = outgoingEdges.filter((edge) => evaluateWhen(edge, state));
          if (selectedEdges.length === 0) {
            selectedEdges = outgoingEdges.filter((edge) => edge.when.op === 'always').slice(0, 1);
          }
        ...
        for (const edge of selectedEdges) {
          ...
          const predecessors = program.edges.filter((e) => e.to === edge.to).map((e) => e.from);
  ...
  private async executeNode(node: ExecNode, state: StateStore): Promise<void> {
    switch (node.op) {
      ...
      case 'SWITCH':
        return;
      case 'TOOL_CALL': {
        const result = await this.toolExecutor.execute(node as ToolCallExecNode);
      ...
      case 'VAR_SET': {
        ...
        const value = isStateRef(varNode.value) ? state.get(varNode.value) : varNode.value;
  ...
  private async executeLlmNode(node: LlmWriteExecNode, state: StateStore): Promise<void> {
