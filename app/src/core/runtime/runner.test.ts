  ...
  it('takes correct switch branch', async () => {
    ...
    const program: ExecProgram = {
      ...
      edges: [
        ...
        { from: 'switch', to: 'yes', when: { op: 'eq', left: '$vars.flag', right: 'yes' } },
        { from: 'switch', to: 'no', when: { op: 'eq', left: '$vars.flag', right: 'no' } },
    ...
    const program: ExecProgram = {
      ...
      nodes: [
        { id: 'start', op: 'START' },
        {
          ...
          prompt_template: 'llm-write-v1',
          output_schema: { type: 'object' },
          validators: [{ type: 'json_schema', schema: { type: 'object', required: ['id'] } }],
