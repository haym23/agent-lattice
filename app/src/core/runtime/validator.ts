...
function evaluateInvariant(expr: string, output: unknown, input: Record<string, unknown>): boolean {
  const membership = /^\$out\.([A-Za-z0-9_]+) in \$in\.([A-Za-z0-9_]+)$/.exec(expr);
  if (membership) {
...
export class Validator {
  validate(output: unknown, node: ExecNode, input: Record<string, unknown> = {}): ValidationResult {
    ...
    if ('validators' in node && Array.isArray(node.validators)) {
      for (const validator of node.validators) {
        if (validator.type === 'json_schema') {
          const schema =
            typeof validator.schema === 'string' ? { $ref: validator.schema } : validator.schema;
        ...
        if (validator.type === 'invariant') {
          ...
          if (!ok) {
            errors.push({ type: 'invariant', message: `Invariant failed: ${validator.expr}` });
          }
        }
      }
    }
