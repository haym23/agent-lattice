...
export class RepairEngine {
  async attemptRepair(
    ...
    const maxAttempts =
      'retry_policy' in node && node.retry_policy ? Math.min(node.retry_policy.max_attempts, 3) : 1;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      const response = await provider.chat({
        ...
        messages: [
          {
            role: 'system',
            content: 'Fix the JSON output based on validation errors. Return JSON only.',
          },
          {
            role: 'user',
