...
function makeWorkflow(overrides: Record<string, unknown> = {}) {
  return {
    ...
    nodes: [
      { id: 'n1', type: 'subAgent' as const, label: 'LLM', position: { x: 0, y: 0 }, config: {} },
    ],
    edges: [],
    ...overrides,
  };
...
describe('compiler pipeline', () => {
  it('emits claude output', () => {
    ...
    const output = compileWorkflow({ target: 'claude', model, workflow: makeWorkflow() });
  ...
  it('throws on empty workflow', () => {
    const model = new ModelRegistry().get('claude-sonnet');
    expect(() =>
      compileWorkflow({ target: 'claude', model, workflow: makeWorkflow({ nodes: [] }) })
  ...
  it('includes warnings for model/target mismatch', () => {
    ...
    const output = compileWorkflow({ target: 'claude', model: gpt, workflow: makeWorkflow() });
    expect(output.warnings.some((w) => w.includes('function-calling'))).toBe(true);
  });
  ...
  it('includes graph warnings for unreachable nodes', () => {
    ...
    const workflow = makeWorkflow({
      nodes: [
        { id: 's', type: 'start' as const, label: 'Start', position: { x: 0, y: 0 }, config: {} },
        {
          id: 'n1',
...
describe('buildAnalyzedGraph', () => {
  it('returns analyzed graph with execution order', () => {
    const model = new ModelRegistry().get('claude-sonnet');
    const graph = buildAnalyzedGraph({ target: 'claude', model, workflow: makeWorkflow() });
