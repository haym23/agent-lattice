...
function describeNode(node: CompileInput['workflow']['nodes'][number]): string {
  ...
  if (node.type === 'subAgent') {
    const description = typeof node.config.description === 'string' ? node.config.description : '';
    const prompt = typeof node.config.prompt === 'string' ? node.config.prompt : '';
    if (description) {
  ...
  if (node.type === 'ifElse' || node.type === 'switch') {
    const target =
      typeof node.config.evaluationTarget === 'string' ? node.config.evaluationTarget : undefined;
    if (target) {
  ...
  if (node.type === 'httpRequest') {
    const method = typeof node.config.method === 'string' ? node.config.method : undefined;
    const url = typeof node.config.url === 'string' ? node.config.url : undefined;
    if (method) {
  ...
  emit(input: CompileInput): CompileOutput {
    const commandBody = [
      ...
      ...input.workflow.nodes.map(
        (node, index) => `${index + 1}. ${node.label} (${node.type})${describeNode(node)}`
      ),
      '',
      'Connections:',
