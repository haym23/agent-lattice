import type { ExecEdge, ExecProgram } from '../../ir/types';
...
function validateGraph(workflow: WorkflowDocument): void {
  ...
  if (starts.length !== 1) {
    throw new Error(`Workflow must contain exactly one start node. Found: ${starts.length}`);
  }
  ...
  if (graph.unreachable.length > 0) {
    throw new Error(`Workflow contains unreachable nodes: ${graph.unreachable.join(', ')}`);
  }
}
...
export function lowerToExecIR(workflow: WorkflowDocument): ExecProgram {
  ...
  const allNodes = new Map<string, WorkflowNode>(workflow.nodes.map((n) => [n.id, n]));
  ...
  const entry = workflow.nodes.find((node) => node.type === 'start');
  if (!entry) {
