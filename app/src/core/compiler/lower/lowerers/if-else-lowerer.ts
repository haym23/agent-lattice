...
export const ifElseLowerer: Lowerer = {
  nodeType: 'ifElse',
  lower(node, context) {
    const evaluationTarget =
      typeof node.config.evaluationTarget === 'string' && node.config.evaluationTarget
        ? node.config.evaluationTarget
        : `$vars.${node.id}.input`;
    const branches = Array.isArray(node.config.branches)
      ? (node.config.branches as Array<{ value?: string; label?: string }>)
      : [];
