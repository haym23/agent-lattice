...
export const variableStoreLowerer: Lowerer = {
  ...
  lower(node) {
    const operation = typeof node.config.operation === 'string' ? node.config.operation : 'set';
    const key =
      typeof node.config.key === 'string' && node.config.key ? node.config.key : `${node.id}.value`;
