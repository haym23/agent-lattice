...
describe('lowerToExecIR', () => {
  it('lowers linear workflow into exec program', () => {
    ...
    expect(output.nodes.map((node) => node.op)).toEqual(['START', 'LLM_WRITE', 'END']);
  ...
  it('throws for unsupported nodes', () => {
    const workflow: WorkflowDocument = {
      ...
      nodes: [
        ...
        { id: 'skill', type: 'skill', label: 'Skill', position: { x: 10, y: 10 }, config: {} },
    ...
    expect(() => lowerToExecIR(workflow)).toThrow('Unsupported workflow node type');
  });
});
