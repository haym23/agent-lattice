...
import { type ExecNode, type ExecProgram, isExecOp, isModelClass, isStateRef } from './types';
...
describe('ir types', () => {
  ...
  it('supports every exec node variant and serializes program', () => {
    const nodes: ExecNode[] = [
      ...
      { id: 'n4', op: 'SWITCH' },
      { id: 'n5', op: 'TOOL_CALL', tool: 'http.request', args: { url: '$vars.url' } },
      { id: 'n6', op: 'VAR_SET', target: '$vars.answer', value: 'ok' },
      { id: 'n7', op: 'VAR_GET', source: '$vars.answer' },
      { id: 'n8', op: 'TRANSFORM', transformation: 'jmespath', expression: 'items[*].id' },
    ];
    ...
    const program: ExecProgram = {
      ...
      edges: [
        ...
        { from: 'n3', to: 'n4', when: { op: 'eq', left: '$vars.answer', right: 'ok' } },
      ],
    };
