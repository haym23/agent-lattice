...
import { type LlmProvider, type LlmRequest, type LlmResponse, MissingApiKeyError } from './types';
  ...
  async chat(request: LlmRequest): Promise<LlmResponse> {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        ...
        return {
          content,
          parsed: request.responseFormat && content ? JSON.parse(content) : undefined,
          usage: {
