...
import { lowerToExecIR } from '../core/compiler/lower';
import { compileWorkflow as compileForEmitter } from '../core/compiler/pipeline';
import type { CompileOutput, CompilerTarget } from '../core/compiler/types';
import type { LlmWriteExecNode } from '../core/ir/types';
import { MockLlmProvider } from '../core/llm/mock-provider';
...
import { createDefaultPromptRegistry } from '../core/prompts/registry';
import { createRunner } from '../core/runtime';
import { PromptCompiler } from '../core/runtime/prompt-compiler';
import type { ExecutionEvent, ExecutionResult } from '../core/runtime/types';
import type { WorkflowDocument } from '../core/workflow/types';
import type { ExecutionListener, McpToolSchema, PlatformAdapter } from './platform-adapter';
...
export class WebPlatformAdapter implements PlatformAdapter {
  ...
  async compileWorkflow(
    ...
    const compiled = compileForEmitter({ workflow, model, target: target as CompilerTarget });
  ...
  async executeWorkflow(
    ...
    const result = await runner.execute(program, input);
