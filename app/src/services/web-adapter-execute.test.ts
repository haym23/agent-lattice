  ...
  it('executes lowered workflow and returns execution result', async () => {
    ...
    const workflow = {
      ...
      nodes: [
        ...
        { id: 'end', type: 'end' as const, label: 'End', position: { x: 120, y: 0 }, config: {} },
      ],
      edges: [{ id: 'e1', source: 'start', target: 'end' }],
    };

    const events: string[] = [];
    const unsub = adapter.subscribeToExecution((event) => events.push(event.type));
  ...
  it('includes runtime prompt preview when compiling workflow with llm nodes', async () => {
    ...
    const workflow = {
      ...
      nodes: [
        ...
        { id: 'end', type: 'end' as const, label: 'End', position: { x: 240, y: 0 }, config: {} },
      ],
      edges: [
    ...
    const result = await adapter.compileWorkflow(workflow, 'claude-sonnet', 'claude');
    const runtimePreview = result.files.find((file) => file.path === '.debug/runtime-prompts.md');
