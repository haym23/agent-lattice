...
const TIERS = [
  {
    tier: "Nano",
    color: C.green,
    models: ["Haiku 3.5", "GPT-4o-mini", "Gemini Flash"],
    tasks: [
      "Classification",
      "Routing",
      "Validation",
  ...
  },
  {
    tier: "Micro",
    color: C.blue,
    models: ["Sonnet 4.5", "GPT-4o", "Gemini Pro"],
    tasks: [
      "Summarization",
  ...
  },
  {
    tier: "Standard",
    color: C.orange,
    models: ["Opus 4.5", "GPT-4.5", "Gemini Ultra"],
    tasks: [
      "Complex reasoning",
      "Creative writing",
      "Architecture",
  ...
  },
  {
    tier: "Heavy",
    color: C.red,
    models: ["Opus 4.6", "o3", "Deep Research"],
    tasks: [
      "Novel research",
      "Entangled reasoning",
  ...
  return (
    <div>
      <Section
        ...
        sub="Five-stage flow from user intent to optimized execution"
      >
      ...
      <Section
        ...
        sub="With block fusion, reasoning traces, and escalation paths"
      >
        <Pre>
          {"USER LAYER\n"}
          {
          ...
          <Clr c={C.green}>[Nano]</Clr> <Clr c={C.blue}>[Micro]</Clr>{" "}
          <Clr c={C.orange}>[Std]</Clr> <Clr c={C.red}>[Heavy]</Clr>
          {"   Model Pools\n"}
          {"\u2500".repeat(60) + "\n"}
          {"RUNTIME\n"}
          {"  Parallel Executor\n"}
          {"  \u251C\u2500 Cache Hit (skip)\n"}
          {"  \u251C\u2500 LLM Call "}
          <Clr c={C.yellow}>+ reasoning_trace: on</Clr>
          {"\n"}
          {"  \u2514\u2500 Quality Gate\n"}
          {"       \u251C\u2500 Pass \u2192 store output + trace\n"}
          {"       \u2514\u2500 Fail \u2192 "}
          <Clr c={C.red}>Escalate</Clr>
          <Clr c={C.yellow}> + attach trace</Clr>
          {"\n"}
          {"  Handoff Manager\n"}
          {"    \u251C\u2500 "}
      ...
      <Section
        title="Block Palette"
        sub="Drag-and-drop primitives with default tier assignments"
      >
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {BT.map((bt) => (
            ...
            >
              ...
              <Badge color={TIERS[bt.dt].color} small>
                {TIERS[bt.dt].tier}
              </Badge>
            </div>
          ))}
...
function FusionTab() {
  var examples = [
    {
      before: [
        ["Classify (Nano)", C.green],
        ["Extract (Nano)", C.green],
        ["Outline (Std)", C.orange],
      ],
      after: [
        ["Classify + Extract (Nano)", C.green],
        ["Outline (Std)", C.orange],
      ],
      rule: "Rule 2 \u2014 serialization between two Nano blocks > 20% overhead",
      savings: "~150 tokens, ~100ms saved",
    },
    {
      before: [
        ["Outline (Std)", C.orange],
      ...
      after: [["Outline + Draft (Std)", C.orange]],
      rule: "Rule 1 \u2014 coherence dependency, draft depends on outline reasoning",
      savings: "~200 tokens, coherence preserved",
    },
    {
      before: [
        ["Research (Heavy)", C.red],
        ["Format (Nano)", C.green],
      ],
      after: [
        ["Research (Heavy)", C.red],
      ...
      rule: "Rule 3 \u2014 tier gap of 3, formatting is 200 tokens, not worth Heavy pricing",
      savings: "No fusion \u2014 saves ~$0.03 per run",
    },
  ]
  return (
    <div>
      <Section
        title="Block Fusion"
        sub="The compiler pass that decides which blocks to merge into single model calls"
      >
      ...
      <Section title="Fusion Decision Algorithm">
        <Pre>
          ...
          {"  // "}
          <Clr c={C.orange}>
            Rule 3: Don\u2019t fuse when tier gap large + cheap block big
          </Clr>
          {"\n"}
          ...
          {"  // "}
          <Clr c={C.pink}>
            Rule 4: Don\u2019t fuse parallel-independent blocks
          </Clr>
          {"\n"}
      ...
      <Section title="Fused Prompt Structure">
        <InfoBox
          title="How a fused multi-block prompt looks to the model"
          color={C.cyan}
        >
          <Pre>
            {"[system]\n"}
  ...
  var traceTypes = [
    {
      ...
      tag: "data-only",
      color: C.green,
      desc: "Pass only the structured output. Use for independent blocks, format transforms, validation.",
      overhead: "+0 tokens",
      example: 'entities: ["AAPL", "Q3", "revenue"]',
    },
    {
      name: "Summary Trace",
      tag: "coherence",
      color: C.yellow,
      desc: "Pass output + compressed reasoning summary. Use for sequential creative/analytical chains.",
      overhead: "+100-300 tokens",
      example:
        'output: "..." reasoning_trace: "Chose narrative structure because input has chronological data. Prioritized Q3 over Q2. Tone: formal."',
    },
    {
      name: "Full Trace",
      tag: "escalation",
      color: C.red,
      desc: "Pass output + full chain-of-thought + failure context. Use for escalations and heavy reasoning handoffs.",
      overhead: "+500-2K tokens",
      example:
        'output: "..." full_trace: "[thinking] Considered three approaches... A fails because... B has edge case... Chose C [/thinking]" escalation_reason: "confidence below threshold"',
    },
  ]
  return (
    <div>
      <Section
        title="Reasoning Trace Propagation"
        sub="Using model reasoning output to maintain continuity across handoffs"
      >
      ...
      <Section title="How Traces Flow Through the DAG">
        <Pre>
          ...
          {"         \u2502 Block C  \u2502  (Micro: Draft) "}
          <Clr c={C.textDim}>{"\u2190 receives outline + WHY"}</Clr>
          {"\n"}
          {
            "         \u2514\u2500\u2500\u2500\u2500\u252C\u2500\u2500\u2500\u2500\u2500\u2518\n"
          }
      ...
      <Section title="Reasoning API Integration">
        <InfoBox
          title="Getting Traces from Different Providers"
          color={C.accent}
        >
          <Pre>
    ...
    <div>
          ...
          {TIERS.map((tier, i) => (
            ...
            >
              ...
              >
                ...
                <span
                  style={{ fontFamily: F.mono, fontSize: 10, color: C.textDim }}
                >
                  {(expanded === i ? "\u25BE" : "\u25B8") + " " + tier.tokens}
                </span>
              </div>
              {expanded === i && (
                <div
      ...
      <Section title="Routing Decision Tree">
        <Pre>
          ...
          {"  \u2514\u2192 complexity < 25?  \u2192  "}
          <Clr c={C.green}>Nano</Clr>
          {"\n"}
          {"  \u2514\u2192 else              \u2192  "}
          <Clr c={C.blue}>Micro</Clr>
          {"\n\n"}
...
function RuntimeTab() {
  var caps = [
    {
      label: "Try-Cheap-First",
      desc: "Run at lowest viable tier, escalate on quality gate failure",
      color: C.green,
    },
    {
      label: "Parallel Fan-Out",
      desc: "Independent blocks execute concurrently across model pools",
      color: C.blue,
    },
    {
      label: "Cache Layer",
      desc: "Content-addressed cache: hash(intent + input) \u2192 stored output",
      color: C.cyan,
    },
    {
      label: "Streaming Assembly",
      desc: "Merge partial outputs as they arrive",
      color: C.orange,
    },
    {
      label: "Escalation + Trace",
      desc: "Failed blocks escalate with reasoning traces for next model",
      color: C.red,
    },
    {
      label: "Agent Switching",
      desc: "Mid-block model swap if confidence/latency bounds exceeded",
      color: C.pink,
    },
  ]
  return (
    <div>
      <Section
        ...
        sub="Execution strategies for the compiled plan"
      >
        <div
      ...
      <Section title="Escalation Flow with Reasoning Traces">
        <Pre>
          {"1. Execute block at assigned tier (e.g., "}
          <Clr c={C.blue}>Micro</Clr>
          {")\n"}
          {"   \u2514\u2500 If trace enabled:\n"}
          ...
          {"   "}
          <Clr c={C.yellow}>
            {"\u251C\u2500 Store trace if downstream blocks need it"}
          </Clr>
          {"\n"}
          {"   \u2514\u2500 Continue to next block\n\n"}
          {"5. If ANY fail \u2192 escalate to next tier ("}
          <Clr c={C.orange}>Standard</Clr>
          {"):\n"}
          {"   "}
          <Clr c={C.red}>
            {"\u251C\u2500 Include failure reason in escalated prompt"}
          </Clr>
          {"\n"}
          {"   "}
          <Clr c={C.red}>
            {"\u251C\u2500 Include full reasoning trace from failed attempt"}
          </Clr>
          {"\n"}
...
function WorkflowTab() {
  var blocks = [
    ...
    },
    {
      ...
      type: "reason",
      tier: 2,
      desc: "Deep reasoning on structure",
      trace: "summary \u2192 next",
      fused: "B",
    },
    {
      ...
      type: "generate",
      tier: 1,
      desc: "Generate from outline + trace",
      trace: "receives trace",
      fused: "B",
    },
    {
      label: "Quality Check",
    ...
    },
    {
      ...
      type: "transform",
      tier: 0,
      desc: "Apply template formatting",
      trace: "none",
      fused: "C",
    },
  ]
  return (
    <div>
      <Section
        title={"Example: Research \u2192 Draft \u2192 Review"}
        sub="6-block workflow showing fusion, routing, and trace decisions"
      >
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {blocks.map((block, i) => {
            var bt = BT.find((b) => b.type === block.type)
            ...
            return (
              ...
              >
                ...
                <Badge color={tier.color} small>
                  {tier.tier}
                </Badge>
                {block.trace !== "none" && (
                  <Badge color={C.yellow} small>
                    {block.trace}
                  </Badge>
                )}
                <Badge color={C.accent} small>
                  {"group " + block.fused}
                </Badge>
              </div>
            )
        ...
        >
          <span style={{ fontFamily: F.mono, fontSize: 10, color: C.green }}>
            {
              "\u2726 After fusion: 6 blocks \u2192 3 execution groups \u00B7 2 handoffs \u00B7 summary trace on B\u2192C boundary"
            }
          </span>
        </div>
      </Section>
      <Section title="Compiled Execution Plan (Post-Fusion)">
        <Pre>
          ...
          {"  "}
          <Clr c={C.green}>
            {"Fused: Classify + Extract \u2192 Nano (Haiku 3.5)"}
          </Clr>
          {"\n"}
          ...
          {"  "}
          <Clr c={C.orange}>
            {"Fused: Research + Draft \u2192 Standard (Opus 4.5)"}
          </Clr>
          {"\n"}
          {"  "}
          ...
          {"  "}
          <Clr c={C.green}>
            {"Fused: Validate + Format \u2192 Nano (Haiku 3.5)"}
          </Clr>
          {"\n"}
          ...
          {"  "}
          <Clr c={C.red}>
            {"Escalation: if validation fails \u2192 re-run Phase 2"}
          </Clr>
          {"\n"}
          {"  "}
          <Clr c={C.red}>
            {"at Heavy tier with: draft, errors, original_trace"}
          </Clr>
          {"\n\n"}
          {"\u2500".repeat(50) + "\n"}
    ...
    >
      ...
      <div style={{ maxWidth: 780, margin: "0 auto" }}>
        <div style={{ marginBottom: 20 }}>
          ...
          >
            {
              "Block-based intent \u2192 compiled DAG \u2192 block fusion \u2192 reasoning traces \u2192 multi-model execution"
            }
          </p>
        </div>
        <div
