...
/**
 * Serializes workflow from canvas.
 */
export function serializeWorkflowFromCanvas(input: {
	...
	const nodes: WorkflowNode[] = input.nodes.map((node) => ({
		...
		label: String(
			(node.data as Record<string, unknown> | undefined)?.label ?? node.id,
		),
...
/**
 * Deserializes workflow to canvas.
 */
